...
//	private static final float MAX_YAW = 90;
//	private static final float MIN_YAW = 0;
	...
	private SmoothFloat angleAroundPlayer = new SmoothFloat(180, 10);
	private SmoothFloat distanceFromPlayer = new SmoothFloat(4, 5);
	...
	public void move(Scene scene) {
		...
		calculateAngleAroundPlayer(player);
	...
	public void focusPlayer(Scene scene){
		angleAroundPlayer.setTarget((scene.getPlayer().getRotation().y + 180) % 360);
	}
	...
	private void calculateCameraPosition(Player player,float horizDistance, float verticDistance) {
		...
		position.x = playerPos.x + (float) (horizDistance * Math.sin(Math.toRadians(theta)));
		position.y = playerPos.y + verticDistance + Y_OFFSET;
		position.z = playerPos.z + (float) (horizDistance * Math.cos(Math.toRadians(theta)));
	}
	 ...
	 */
	private void calculateAngleAroundPlayer(Player player) {
		if (Mouse.isButtonDown(0)) {
